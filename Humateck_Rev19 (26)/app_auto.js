...
function buildOrderSheet(plan, title, description, videoUrl) {
  ...
  return [
    ...
    "",
    "안내:",
    "위 주문 내용을 확인하신 후 자동번역 요청 버튼을 눌러 주십시오."
  ].join("\n");
}
...
function resetOrderSheet() {
  ...
  setStatus("제출하기를 누르면 플랜에 맞는 AUTO 주문서가 먼저 생성됩니다.");
}
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  $("submitBtn")?.addEventListener("click", () => {
    ...
    if ($("orderSheetOutput")) {
      $("orderSheetOutput").value = buildOrderSheet(
        payload.plan,
        payload.title,
        payload.description,
        payload.videoUrl
  ...
  $("resetBtn")?.addEventListener("click", () => {
    ["videoUrl", "titleInput", "descInput"].forEach((id) => {
      if ($(id)) $(id).value = "";
    });
    applyPlanInfo();
    resetOrderSheet();
